...
class AIEngine:
    ...
    def confirmation_command(self, command: str, status, current_branch, recent_commits, command_risk_level: str, dry_output: str, dry_mode) -> str:
        """
...
"""
    ...
    def _post_process(self, response: str, risk_level: str):
        ...
        # Remove alternatives if not dangerous
        if risk_level != "DANGEROUS" and "[ALTERNATIVES]" in response:
            response = response.split("[ALTERNATIVES]")[0].strip()

        # Remove duplicate tips
        if "[TIPS]" in response:
            parts = response.split("[TIPS]")
